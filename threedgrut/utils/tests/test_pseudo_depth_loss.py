...
from threedgrut.utils.pseudo_depth_loss import compute_pseudo_depth_order_loss, disparity_scale, sample_pair_offset
...
def test_agreeing_order_costs_nothing():
    ...
    loss = compute_pseudo_depth_order_loss(
        pred_dist, pred_opacity, prior, scene_extent=1.0, rng=_horizontal_offset_rng()
    )
...
def test_reading_the_prior_as_depth_inverts_the_loss():
    ...
    good = compute_pseudo_depth_order_loss(
        pred_dist, pred_opacity, consistent, scene_extent=1.0, rng=_horizontal_offset_rng()
    )
    bad = compute_pseudo_depth_order_loss(
        pred_dist, pred_opacity, mistaken, scene_extent=1.0, rng=_horizontal_offset_rng()
    )
...
def test_invariant_to_monotone_transforms_of_the_prior():
    ...
    baseline = compute_pseudo_depth_order_loss(
        pred_dist, pred_opacity, prior, scene_extent=1.0, gate=0.0, rng=_horizontal_offset_rng()
    )
    for transformed in (3.0 * prior + 7.0, prior.exp(), prior**3):
        loss = compute_pseudo_depth_order_loss(
            pred_dist, pred_opacity, transformed, scene_extent=1.0, gate=0.0, rng=_horizontal_offset_rng()
        )
...
def test_gate_discards_pairs_tied_relative_to_the_maps_spread():
    ...
    kept = compute_pseudo_depth_order_loss(
        pred_dist, pred_opacity, prior, scene_extent=1.0, gate=0.2, rng=_horizontal_offset_rng()
    )
    dropped = compute_pseudo_depth_order_loss(
        pred_dist, pred_opacity, prior, scene_extent=1.0, gate=0.5, rng=_horizontal_offset_rng()
    )
...
def test_gate_is_invariant_to_rescaling_the_prior():
    ...
    rescaled = compute_pseudo_depth_order_loss(
        pred_dist, pred_opacity, 1e-4 * prior + 5.0, scene_extent=1.0, gate=0.2, rng=_horizontal_offset_rng()
    )
...
def test_scene_extent_scales_the_loss():
    ...
    scaled = compute_pseudo_depth_order_loss(
        pred_dist, pred_opacity, prior, scene_extent=4.0, rng=_horizontal_offset_rng()
    )
...
def test_shape_mismatch_is_rejected():
    ...
    with pytest.raises(ValueError, match="resampled to the training resolution"):
        compute_pseudo_depth_order_loss(
            pred_dist, pred_opacity, torch.zeros(1, 4, 4, 1), scene_extent=1.0, rng=_horizontal_offset_rng()
        )
...
def test_non_positive_scene_extent_is_rejected():
    ...
    with pytest.raises(ValueError, match="scene_extent must be positive"):
        compute_pseudo_depth_order_loss(pred_dist, pred_opacity, prior, scene_extent=0.0)
...
def test_pairs_do_not_wrap_around_the_image():
    ...
    for dy, dx in ((0, 5), (0, -5), (5, 0), (3, 3)):
        loss = compute_pseudo_depth_order_loss(
            pred_dist, pred_opacity, prior, scene_extent=1.0, gate=0.0, rng=_ScriptedRng(dy, dx)
        )
        assert loss.item() == pytest.approx(0.0), f"offset {(dy, dx)} wrapped around"


def test_disparity_scale_is_the_interquartile_range():
    values = torch.arange(101, dtype=torch.float32)
    assert disparity_scale(values).item() == pytest.approx(50.0, abs=1e-4)


def test_disparity_scale_ignores_non_finite_entries():
    values = torch.tensor([float("nan"), 0.0, 1.0, 2.0, 3.0, 4.0, float("inf")])
    assert torch.isfinite(disparity_scale(values))


def test_disparity_scale_of_empty_input_is_zero():
    assert disparity_scale(torch.tensor([float("nan")])).item() == pytest.approx(0.0)
