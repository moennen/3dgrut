...
class FeatureType(Enum):
    """Enum for feature representation types."""
    SH = "sh"  # Spherical harmonics
    NHT = "nht"  # Neural harmonic texture (K-dim features -> decoder -> 3 RGB)
    ...
    def from_string(cls, value: str) -> "FeatureType":
        ...
        for member in cls:
            if member.value == value_lower:
                return member
        raise ValueError(f"Invalid feature_type: '{value}'. Must be one of {[m.value for m in cls]}")


class MixtureOfGaussians(torch.nn.Module, ExportableModel):
    """ """

    def _nht_num_interpolation_points(self) -> int:
        nht = self.conf.model.nht_features
        interp = getattr(nht, "interpolation_type", "none").lower()
        return 1 if interp == "none" else 4

    def _nht_dims(self) -> tuple[int, int]:
        nht = self.conf.model.nht_features
        dim = int(nht.dim)
        num_points = self._nht_num_interpolation_points()
        if dim % num_points != 0:
            raise ValueError(
                f"nht_features.dim={dim} must be divisible by num_interpolation_points={num_points} "
                f"(interpolation_type + primitive)"
            )
        per_point_dim = dim // num_points
        act = getattr(nht, "activation", None)
        act_type = getattr(act, "type", "none").lower() if act is not None else "none"
        if act is None or act_type == "none" or act_type == "relu":
            num_freq = 1
        else:
            num_freq = int(getattr(act, "num_frequencies", 1))
        return dim, per_point_dim * num_freq
    ...
    def feature_fields(self) -> list[str]:
        ...
        if self.feature_type == FeatureType.SH:
            return [
                "features_albedo",
                "features_specular",
            ]
        elif self.feature_type == FeatureType.NHT:
            return ["features"]
        else:
    ...
    def get_features_albedo(self) -> torch.Tensor:
        if self.feature_type == FeatureType.SH:
            ...
            raise AttributeError(f"features_albedo not available in feature_type='{self.feature_type.value}' mode")

    def get_features_specular(self) -> torch.Tensor:
        if self.feature_type == FeatureType.SH:
            return self.features_specular
        else:
            raise AttributeError(f"features_specular not available in feature_type='{self.feature_type.value}' mode")
    ...
    def get_model_parameters(self) -> dict:
        ...
        model_params = {
            ...
            "feature_type": self.feature_type.value,  # Store as string for serialization
    ...
    def __init__(self, conf, scene_extent=None):
        ...
        self.feature_type = FeatureType.from_string(self.conf.model.feature_type)

        primitive_type = (getattr(conf.render, "primitive_type", None) or "").lower()
        if self.feature_type == FeatureType.NHT and primitive_type == "trisurfel":
            raise ValueError(
        ...
        if self.feature_type == FeatureType.SH:
            ...
            self.nht_num_interpolation_points = self._nht_num_interpolation_points()
            self.particle_feature_dim, self.ray_feature_dim = self._nht_dims()
    ...
    def init_from_pretrained_point_cloud(self, pc_path: str, set_optimizable_parameters: bool = True):
        if self.feature_type != FeatureType.SH:
            raise NotImplementedError(f"init_from_pretrained_point_cloud only supports feature_type='sh', got '{self.feature_type.value}'")
    ...
    def init_from_random_point_cloud(
        ...
        if self.feature_type == FeatureType.SH:
            ...
            # Initialize learned features with random values
    ...
    def init_from_checkpoint(self, checkpoint: dict, setup_optimizer=True):
        ...
        checkpoint_feature_type = FeatureType.from_string(checkpoint_feature_type_str)
        ...
        if checkpoint_feature_type == FeatureType.NHT:
    ...
    def default_initialize_from_points(self, pts, observer_pts, colors=None, use_observer_pts=True):
        ...
        if self.feature_type == FeatureType.SH:
            ...
            act_type = self.conf.model.nht_features.activation.type.lower()
            if act_type in ("siren", "sincos"):
                # Uniform [0, pi/2] so sin(base*2^f) has good gradient flow (gsplat-style).
                features = (
    ...
    def setup_optimizer(self, state_dict=None):
        ...
        for name, args in self.conf.optimizer.params.items():
            ...
            if not hasattr(self, name):
                logger.info(f"Skipping optimizer parameter '{name}' - not present in {self.feature_type.value} mode")
                continue
    ...
    def forward(
        self, batch: Batch, train=False, frame_id=0, return_features_first3: bool = False
        ...
        Args:
            ...
            return_features_first3: if True, include pred_features_first3 (first 3 features before MLP/SH eval)
        Returns:
            A dictionary containing the output of the model
        """
        return self.renderer.render(self, batch, train, frame_id, return_features_first3=return_features_first3)
    ...
    def init_from_ply(self, mogt_path: str, init_model=True):
        if self.feature_type != FeatureType.SH:
            raise NotImplementedError(f"init_from_ply only supports feature_type='sh', got '{self.feature_type.value}'")
    ...
    def copy_fields(self, other, deepcopy=False):
        ...
        if deepcopy:
            ...
            if other.feature_type == FeatureType.SH:
            ...
            elif other.feature_type == FeatureType.NHT:
