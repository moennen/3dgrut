...
def markdown(records: list[dict], note: str) -> str:
    ...
    text = [
        ...
        "```",
        "",
        "Omit `--max-frames` and `--max-image-side` for the full-resolution multi-view run. The default "
        "surface sampler uses two million uniform mesh samples; keep `--gt-voxel` unset for benchmark scoring. "
        "The MoGe-3 default checkpoint is `/mnt/oss/MoGe/checkpoints/moge-3-vitl/model.pt`.",
        "",
        "The output directory contains `results.jsonl` (one complete cell per model/suite/alignment), "
        "`protocol.json`, aligned depth maps, visibility z-buffers, and TSDF meshes. Regenerate this tracker and "
        "the slide deck with:",
        "",
        "```bash",
