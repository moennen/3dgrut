...
| D05 | Feature precision | Trainable NHT features and decoder inputs are fp32 Python-side, but the NHT rasterizer casts feature colors to half and launches `at::Half` kernels; loaded feature values are converted back to float for accumulation. | Current benchmark also uses `particle_feature_half=true` and `feature_output_half=true` at the tracer boundary. | Lower than originally suspected as a parity issue, but important in 3dgrut: disabling the boundary/output half path improved E04 by +0.3670 dB over E03. | Measured |
