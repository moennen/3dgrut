...
| D05 | Feature precision | Trainable NHT features and decoder inputs are fp32 Python-side, but the NHT rasterizer casts feature colors to half and launches `at::Half` kernels; loaded feature values are converted back to float for accumulation. Backward accumulates color gradients in fp32 and returns them in the original input dtype. | 3dgrut now keeps trainable feature tensors fp32 across the Python/custom-autograd boundary. When `PARTICLE_FEATURE_HALF` is enabled, the 3DGUT and 3DGRT C++ bindings create a transient contiguous fp16 kernel tensor before launching the renderer, while gradients remain fp32. | Important in 3dgrut, but not due to the final ray-feature output dtype: E04a showed `feature_output_half=false` alone gives no PSNR recovery, while E04b recovered PSNR with `particle_feature_half=false` even when `feature_output_half=true`. The implemented fix preserves gsplat-like fp16 kernel reads while avoiding the lossy trainable-feature autograd boundary. | Fixed |
...
| --- | --- | --- | --- |
| 2026-06-08 | D05 | Removed Python-side NHT feature `.half()` casts from 3DGUT and 3DGRT tracer calls. Added C++ binding-side feature casts to match gsplat: trainable features stay fp32 for autograd, kernels read fp16 when `PARTICLE_FEATURE_HALF=1`, and feature gradients are accumulated/returned in fp32. | `clang-format --dry-run --Werror threedgut_tracer/src/splatRaster.cpp threedgrt_tracer/src/optixTracer.cpp`; `python -m py_compile threedgut_tracer/tracer.py threedgrt_tracer/tracer.py`; `git diff --check`; 10-step bonsai smoke with `render.particle_feature_half=true render.feature_output_half=true` completed train and render. |
...
| 2026-06-08 | E03 rerun | `79c979e2` | `MODE=3dgrut EXP_ID=e03_rerun_after_feature_half_fix MAX_STEPS=7000 FEATURE_DIM=48 CAP_MAX=1000000 TORCH_EXTENSIONS_DIR=results/nht_gsplat_parity/.torch_extensions_3dgrut_e03_after_feature_half_fix initialization.use_observation_points=false dataset.normalize_world_space=true` | 31.0023 | 0.9322 | 0.2819 | 10.38 ms | Rerun of the E03 cumulative config after fixing D05 to keep trainable features fp32 across the custom-autograd boundary while preserving fp16 kernel reads. This is +0.4361 dB over the old E03 and within run noise of E00 gsplat. Training: 1066.41 s, 6.56 it/s. Metrics: `results/nht_gsplat_parity/e03_rerun_after_feature_half_fix/3dgrut_current/bonsai/eval/bonsai/bonsai-0806_083229/metrics.json`. |
