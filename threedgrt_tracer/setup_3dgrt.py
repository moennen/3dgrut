...
def setup_3dgrt(conf):
    ...
    def _get_particle_feature_dim(conf):
        """Total feature dim per particle (K, buffer stride). For nht = nht_features.dim. Not per-interpolation-point (that is INTERP_POINT_FEATURE_DIM)."""
        feature_type = conf.model.feature_type.lower()
        if feature_type == "sh":
    ...
    def _get_interpolation_type(conf):
        """NHT: 0=barycentric (or center), 1=bezier. Merged with support: none=center, barycentric=vertex interp."""
        feature_type = conf.model.feature_type.lower()
        if feature_type != "nht":
            return 0
    ...
    subprocess.check_call(
        [
            ...
            f"{os.path.join(slang_build_file_dir,'models/radiativeParticles.slang')}",
            "-o",
